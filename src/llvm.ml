...
let rec compile_llvm e label = 
  match e with 
  | Num x -> Const x, label, [],[]
  | Bool b when b = true -> Const 1, label, [], []
  | Bool b when b = false -> Const 0, label, [], []
  | Add (_,e1,e2) -> 
    let r1,l1,b1,bs1 = compile_llvm e1 label in
    let r2,l2,b2,bs2 = compile_llvm e2 l1 in
    let ret = new_reg() in 
    (Register ret, l2, b1@b2@[Addi32 (ret,r1,r2)], bs1@bs2)
  | And (_,e1,e2) -> 
    let r1,l1,b1,bs1 = compile_llvm e1 label in
...
let emit_printf ret ty = 
  let t = begin match ty with
          | IntT -> "i32"
          | BoolT -> "i1"
          | _ -> failwith "Do not know this type..."
          end
  in
    "  "^unparse_register (new_reg())^" = call i32 (i8*, ...) @printf(i8* noundef @.str, "^t^" noundef "^unparse_result ret^")"
